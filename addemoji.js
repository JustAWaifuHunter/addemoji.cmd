  ...
  */
  run: async (client, message, args) => {
    ...
    if (!message.member.permissions.has(Discord.Permissions.FLAGS.MANAGE_EMOJIS_AND_STICKERS)) {
      return message.channel.send({ content: "<:No_sabri:882037308453441606> Você não pode utilizar este comando! \n\n <:Info_sabri:882037503022993428> Ei pisiu Caso queira usar esse comando" })
    }
    
    const emoji = args[0];

    if (!emoji) return message.channel.send({ content: `<:errr_sabri:882037933052395570> Desculpe não consegui achar um emoji! \n** <:Info_sabri:882037503022993428> Por favor me envie um emoji existente.**` })
    ...
    if (customemoji.id) {
      ...
      ), Added = new Discord.MessageEmbed()
        .setTitle(`
      ...
      return message.channel.send({ content: `<:errr_sabri:882037933052395570> Desculpe não consegui achar um emoji! \n** <:Info_sabri:882037503022993428> Por favor me envie um emoji existente.**` })     
    }
  }
};
